...
- **图片文件大小别担心**：Astro `<Figure>` 走 sharp 管道自动转 webp + 压缩；2MB 的 PNG 出来 ~50KB。直接丢原图就行，不用提前压。
- **别放敏感信息**：frontmatter 或正文里不要写 API key、个人邮箱之外的 secret
- **tags 要用英文 + 连字符**：`computational-geometry` 而不是 `ComputationalGeometry` 或 `计算几何`
- **新文章如果有罕见 CJK 字符**：可能正文显示成 "豆腐块"。这时跑 `npm run subset-fonts` 重新生成字体子集，再 push。常用字 + 标点已经在 safety buffer 里，多数文章不需要这一步。
